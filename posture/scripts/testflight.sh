...
if [[ -f "$PROJECT_YML" ]]; then
    ...
    if [[ -n "$CURRENT_BUILD" && "$CURRENT_BUILD" =~ ^[0-9]+$ ]]; then
        ...
        # ~ is a single git repo containing multiple projects — stage explicitly.
        ( cd "$(dirname "$ROOT")" && git add "$(basename "$ROOT")/project.yml" \
            && git commit -m "chore(posture): bump build $CURRENT_BUILD -> $NEW_BUILD" || true )
        ( cd "$(dirname "$ROOT")" && git push origin main || true )
    else
